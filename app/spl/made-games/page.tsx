...
import { getRPSSplBetsByMaker } from '../../server/spl/getRPSSplBetsByMaker';
...
export default function MySplRPSBets() {
  ...
  const [myBets, setMyBets] = useState<DecryptedSplBet[]>([]);
  ...
  useEffect(() => {
    async function fetchAndDecryptBets() {
      ...
      const result = await getRPSSplBetsByMaker(publicKey.toString());
      if (result.success) {
        ...
        setError(result.error || 'Failed to fetch your SPL bets');
      }
      setIsLoading(false);
    }
    fetchAndDecryptBets();
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-grow container mx-auto px-4 py-8">
        ...
        {!publicKey && <p className="text-center">Please connect your wallet to view your SPL games.</p>}
        {isLoading && <p className="text-center">Loading your SPL games...</p>}
